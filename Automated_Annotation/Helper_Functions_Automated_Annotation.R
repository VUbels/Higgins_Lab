...
read_all_mtx_and_create_seurat <- function(directory) {
  ...
  for (base in base_names) {
    ...
    if (file.exists(mtx_file) && file.exists(features_file) && file.exists(cells_file)) {
      ...
      seurat_object <- subset(seurat_object, subset = nFeature_RNA > NFeaturesRNA & percent.mt < Mito_Cutoff)
...
assign_disease_status <- function(ident, tags){
  # Create the dynamic regular expression
  pattern <- paste0("_(", paste(tags, collapse="|"), ")(?=\\d)")
  
  # Use the regular expression to match the desired pattern
  match <- stringr::str_extract(ident, pattern)
  
  # If there's a match, return it, otherwise return NA
  if(!is.na(match)){
    return(gsub("^_", "", match))
  } else {
    return(NA)
